...
function reconstructed_image = ...
    BCS_SPL_DDWT_Decoder1(y, num_rows, num_cols, num_levels, max_iterations, A, At)
...
for i = 1:max_iterations
  [x D] = SPLIteration(y, x, num_rows, num_cols, ...
      lambda, num_levels,[],A,At);
...
function [x2 D] = SPLIteration(y, x, num_rows, num_cols, ...
    lambda, num_levels, last, A, At)
...
%x_hat = x_hat + reshape(At( (y - A( x_hat))),[512,512]);
x_hat = x_hat + reshape(At( (y - A( x_hat))),[num_rows, num_cols]);
x1=x_hat;
...
%x2 = x_bar + reshape(At( (y - A( x_bar))),[512,512]);
x2 = x_bar + reshape(At( (y - A( x_bar))),[num_rows, num_cols]);
