...
def RCAN(t_image, is_train=False, reuse=False):     # multi level non-useful
    ...
    with tf.variable_scope("RCAN", reuse=reuse):
        ...
        n = Conv2d(n, 64, (5, 5), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='conv1')
        nn = n
        ...
        n = Conv2d(n,  1, (5, 5), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='conv3')
...
def ResidualGroup(n):
    ...
    nn = Conv2d(nn, 64, (5, 5), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='conv')
    n = ElementwiseLayer([n, nn], tf.add, name='add')
...
def RCAB(n, rat):
    ...
    nn = n
    nn = AtrousConv2dLayer(nn, n_filter = 64, filter_size = (5, 5), rate = rat, act=tf.nn.relu, padding='SAME', W_init=w_init, b_init=b_init, name='conv1')
    nn = AtrousConv2dLayer(nn, n_filter = 64, filter_size = (5, 5), rate = rat, act=tf.identity, padding='SAME', W_init=w_init, b_init=b_init, name='conv2') #tf.identity means no act
