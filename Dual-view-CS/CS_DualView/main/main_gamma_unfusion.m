...
for denoize_choice = [26]  % [4,19,22,23]
    for set = 14 %[12,13]
        ...
        for kk= 1%:6
            ...
            for k = 1:int32(img_num)
                ...
                fprintf('Denoiser:%s, Rate:%f, measure_model:%s ,gamma=unfusion\n',measure_1.denoize_name, measure_1.rate, measure_1.model);
                ...
                fp = fopen(['../results/', Test_set{set}, '/', measure_1.denoize_name,'_multi_gamma_unfusion.csv'],'a');
            ...
            fp = fopen(['../results/', Test_set{set}, '/', measure_1.denoize_name,'_average','_multi_gamma_unfusion.csv'],'a');   %测试集所有图片在不同bpp下的平均R-D
            fprintf(fp,'%s, %s, %f,%f,%f,%f\n', measure.Image_name_1, measure_1.denoize_name,measure_2.rate,PSNR_eve, SSIM_eve,MSE_eve);
            fclose(fp);
        end
        fp = strcat('../results/', Test_set{set}, '/','/','ablation_',measure_1.denoize_name,'fix_gamma=unfusion','.csv');
