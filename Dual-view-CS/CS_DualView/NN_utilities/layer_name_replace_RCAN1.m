...
fileFolder = fullfile('../../Trained_Weights/RCAN_5x5_17_dilated/');
dir_name = dir(fullfile(fileFolder,'*.mat'));
dir_number = length(dir_name);
for j = 1:length(dir_name)
    load(fullfile('../../Trained_Weights/RCAN_5x5_17_dilated/',dir_name(j).name));
    ...
    if k_t~=0
        save(fullfile(fileFolder, dir_name(j).name),'net');
    end
